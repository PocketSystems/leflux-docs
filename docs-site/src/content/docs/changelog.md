...
- **Smarter Navigation**: The chat widget now recognizes when it has already tried to move you to another page and will stop suggesting the same action repeatedly.
...
- **Better language detection**: Your visitors will now receive replies in the expected alphabet when they type in languages like Roman Urdu.
- **Improved internal linking**: The widget can now find and link to more relevant pages on your site, making navigation smoother for your users.
...
- **Instant Owner Controls**: Manage your widget settings directly without extra steps or pop-up windows.
...
- **Smoother tab switching**: Switching between tabs in your dashboard is now instant and seamless, no more waiting or jumping layouts.
- **Polished design**: The entire dashboard has a refreshed and consistent look, with improved controls and readability.
- **Save feedback**: The save button now clearly shows its state, preventing layout shifts after you make changes.
- **Visual consistency**: Your settings for the widget’s appearance now match the look and feel of the Dashboard, making it easier to set up and recognize.
- **Improved Corner Selection**: Easily choose the widget's corner position with a visual dot on a screen preview.
- **Enhanced Offset Controls**: Adjust widget offsets precisely using a new stepper with plus and minus buttons, and scroll wheel support.
### Server
